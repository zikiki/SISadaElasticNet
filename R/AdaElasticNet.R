...
AdaElasticNet = function (x, y, family = c("gaussian", "binomial", "poisson",
                           "cox"), init = c("enet", "ridge"), alphas = seq(0.05, 0.95,
                                                                           0.05), tune = c("cv", "ebic", "bic", "aic"), nfolds = 5L,
          rule = c("lambda.min", "lambda.1se"), ebic.gamma = 1, scale = 1,
          lower.limits = -Inf, upper.limits = Inf, seed = 1001, parallel = FALSE,
          verbose = FALSE)
{
  ...
  if (verbose)
    cat("Starting step 1 ...\n")
  if (init == "enet") {
    enet.cv = tune.glmnet(x = x, y = y, family = family,
                                  alphas = alphas, tune = tune, nfolds = nfolds, rule = rule,
                                  ebic.gamma = ebic.gamma, lower.limits = lower.limits,
                                  upper.limits = upper.limits, seed = seed, parallel = parallel)
  }
  if (init == "ridge") {
    enet.cv = tune.glmnet(x = x, y = y, family = family,
                                  alphas = 0, tune = tune, nfolds = nfolds, rule = rule,
                                  ebic.gamma = ebic.gamma, lower.limits = lower.limits,
                                  upper.limits = upper.limits, seed = seed, parallel = parallel)
  }
  ...
  if (verbose)
    cat("Starting step 2 ...\n")
  aenet.cv = tune.glmnet(x = x, y = y, family = family,
                                 alphas = alphas, tune = tune, nfolds = nfolds, rule = rule,
                                 ebic.gamma = ebic.gamma, lower.limits = lower.limits,
                                 upper.limits = upper.limits, seed = seed + 1L, parallel = parallel,
  ...
  aenet.full = glmnet(x = x, y = y, family = family, alpha = best.alpha.aenet,
                      lambda = best.lambda.aenet, lower.limits = lower.limits,
                      upper.limits = upper.limits, penalty.factor = adpen)
  bhat.full = Matrix(aenet.full$beta, sparse = TRUE)
  aenet.model = list(beta = bhat.full, model = aenet.full,
                     beta.first = enet.full$beta, model.first = enet.full,
                     best.alpha.enet = best.alpha.enet, best.alpha.aenet = best.alpha.aenet,
                     best.lambda.enet = best.lambda.enet, best.lambda.aenet = best.lambda.aenet,
                     step.criterion = c(step.criterion.enet, step.criterion.aenet),
                     adpen = adpen, seed = seed, call = call)
